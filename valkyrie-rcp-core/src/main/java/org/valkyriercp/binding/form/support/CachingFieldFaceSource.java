...
public abstract class CachingFieldFaceSource implements FieldFaceSource {
	...
	/*
	 * A cache with context keys and Map from field to FieldFace values. The
	 * keys are held with week references so this class will not prevent GC of
	 * context instances.
	 */
	private final CachingMapDecorator cachedFieldFaceDescriptors = new org.valkyriercp.util.CachingMapDecorator(
			true) {
	...
	public FieldFace getFieldFace(final String field, final Object context) {
		Map faceDescriptors = (Map) cachedFieldFaceDescriptors
				.get(context == null ? DEFAULT_CONTEXT : context);
	...
	/**
	 * Loads the FieldFace for the given field path and context id. This value
