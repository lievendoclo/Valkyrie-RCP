 ...
 * @author Arne Limburg
 */
public class DefaultMemberPropertyAccessor extends
	...
	public DefaultMemberPropertyAccessor(Class targetClass, Object target,
			boolean fieldAccessEnabled, boolean strictNullHandlingEnabled) {
	...
	protected DefaultMemberPropertyAccessor(
	...
	public void setTarget(Object target) {
		if (getParentPropertyAccessor() != null) {
			throw new IllegalStateException(
					"explicite setting of target is not allowed for child property accessors");
		}
		this.target = target;
		if (!fixedTargetClass && target != null
				&& target.getClass() != getTargetClass()) {
	...
	public Object getIndexedPropertyValue(String propertyName)
		...
		if (getPropertyType(propertyName) == null) {
			throw new NotReadablePropertyException(getTargetClass(),
					propertyName, "property type could not be determined");
		}
	...
	public Object getSimplePropertyValue(String propertyName)
		...
		try {
			...
			throw new InvalidPropertyException(getTargetClass(), propertyName,
					"Property is not accessible", e);
	...
	private Object getPropertyValue(Object assemblage, Object[] indices,
		...
		if (assemblage.getClass().isArray()) {
			...
			value = getListValue((List) assemblage,
					(Integer) indices[parameterIndex]);
			...
			value = getCollectionValue((Collection) assemblage,
					(Integer) indices[parameterIndex]);
		} else {
			throw new IllegalStateException(
		...
		if (map.containsKey(key)) {
			...
			if (!(JdkVersion.getMajorJavaVersion() >= JdkVersion.JAVA_15)) {
				...
				for (Iterator i = map.entrySet().iterator(); i.hasNext();) {
					...
					if (entry.getKey() == key
							|| (entry.getKey() != null && key != null && entry
									.getKey().toString().equals(key.toString()))) {
	...
	public void setIndexedPropertyValue(String propertyName, Object value)
			throws BeansException {
		...
		try {
			...
			throw new NotWritablePropertyException(getTargetClass(),
					propertyName, "parent property is not readable", e);
		}
		if (parentValue == null) {
		...
		try {
			...
			throw new NotWritablePropertyException(getTargetClass(),
					propertyName, "wrong index type", e);
		}
		Object index = indices[indices.length - 1];
		Object newParentValue = setAssemblageValue(
				getPropertyType(parentPropertyName), parentValue, index, value);
		if (newParentValue != parentValue) {
			setPropertyValue(parentPropertyName, newParentValue);
		}
	}

	public void setSimplePropertyValue(String propertyName, Object value)
			throws BeansException {
	...
	protected AbstractNestedMemberPropertyAccessor createChildPropertyAccessor(
			String propertyName) {
	...
	public static class ReflectionUtils extends
