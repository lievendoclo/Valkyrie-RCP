 ...
 * 
 * <pre>
 * private EventListenerListHelper fooListeners = new EventListenerListHelper(
 * 		FooListener.class);
 * 
...
public class EventListenerListHelper implements Serializable {
	...
	private static final Map methodCache = new CachingMapDecorator() {
		...
		/**
		 * Creates a value to cache under the given key {@code o}, which must be
		 * a {@link MethodCacheKey}. The value to be created will be a
		 * {@link java.lang.reflect.Method} object that is specified by the
		 * given key.
		 * 
		 * @param o
		 *            The key that the newly created object will be stored
		 *            under. This is expected to be an instance of
		 *            {@link MethodCacheKey} that contains the class, method
		 *            name and number of parameters of the
		 *            {@link java.lang.reflect.Method} to be created.
		 * 
		 * @throws ClassCastException
		 *             if {@code o} can not be assigned to
		 *             {@link MethodCacheKey}.
		 * @throws IllegalArgumentException
		 *             if the listener class specified by {@code o}, does not
		 *             have an implementation of the method specified in the
		 *             given key.
		 */
		protected Object create(Object o) {
			...
			if (fireMethod == null) {
				throw new IllegalArgumentException("Listener class ["
						+ key.listenerClass + "] does not implement method ["
						+ key.methodName + "] with [" + key.numParams
						+ "] parameters.");
			}
			return fireMethod;
		}
	...
	/**
	 * Create new <code>EventListenerListHelper</code> instance that will
	 * maintain a list of event listeners of the given class.
	 * 
	 * @param listenerClass
	 *            The class of the listeners that will be maintained by this
	 *            list helper.
	 * 
	 * @throws IllegalArgumentException
	 *             if {@code listenerClass} is null.
	 * 
	 */
	 ...
	 * The returned iterator does not allow removal of listeners. To remove a
	 * listener, use the {@link #remove(Object)} method.
	 * 
	 ...
	 * 
	 * @throws IllegalArgumentException
	 *             if no method with the given name and an empty parameter list
	 *             exists on the listener class maintained by this list helper.
	 */
	...
	/**
	 * Invokes the method with the given name and a single parameter on each of
	 * the listeners registered with this list.
	 * 
	 * @param methodName
	 *            the name of the method to invoke.
	 * @param arg
	 ...
	 * 
	 * @throws IllegalArgumentException
	 *             if no method with the given name and a single formal
	 *             parameter exists on the listener class managed by this list
	 *             helper.
	 */
	 ...
	 * 
	 * @throws IllegalArgumentException
	 *             if no method with the given name and 2 formal parameters
	 ...
	 * 
	 * @throws IllegalArgumentException
	 *             if no method with the given name and number of formal
	 *             parameters exists on the listener class managed by this list
	 *             helper.
	 */
	 ...
	 * 
	 * @return true if the listener was registered, false if {@code listener}
	 *         was null or it is already registered with this list helper.
	 * 
	 * @throws IllegalArgumentException
	 *             if {@code listener} is not assignable to the class of
	...
	/**
	 * Adds all the given listeners to the list of registered listeners. If any
	 * of the elements in the array are null or are listeners that are already
	 * registered, they will not be registered again.
	 * 
	 * @param listenersToAdd
	 *            The collection of listeners to be added. May be null.
	 * 
	 ...
	 * 
	 * @throws IllegalArgumentException
	 *             if any of the listeners in the given collection are of a type
	 *             that is not assignable to the class of listener that this
	 *             instance manages.
	 */
	 ...
	 * 
	 * @throws IllegalArgumentException
	 *             if {@code listener} is null or not assignable to the class of
	 *             listener that is maintained by this instance.
	 */
	...
	/**
	 * Invokes the method with the given name on each of the listeners
	 * registered with this list helper. The given arguments are passed to each
	 * method invocation.
	 * 
	 * @param methodName
	 *            The name of the method to be invoked on the listeners.
	 * @param eventArgs
	 *            The arguments that will be passed to each method invocation.
	 *            The number of arguments is also used to determine the method
	 *            to be invoked.
	 * 
	 * @throws EventBroadcastException
	 *             if an error occurs invoking the event method on any of the
	 *             listeners.
	 */
	private void fireEventByReflection(String methodName, Object[] eventArgs) {
		Method eventMethod = (Method) methodCache.get(new MethodCacheKey(
				listenerClass, methodName, eventArgs.length));
		Object[] listenersCopy = listeners;
		for (int i = 0; i < listenersCopy.length; i++) {
			try {
				eventMethod.invoke(listenersCopy[i], eventArgs);
			} catch (InvocationTargetException e) {
				throw new EventBroadcastException(
						"Exception thrown by listener", e.getCause());
			} catch (IllegalAccessException e) {
				throw new EventBroadcastException("Unable to invoke listener",
						e);
			}
		}
	}
	...
	public static class EventBroadcastException extends NestedRuntimeException {
		...
		/**
		 * Creates a new {@code EventBroadcastException} with the given detail
	...
	/**
	 * Returns an object which is a copy of the collection of listeners
