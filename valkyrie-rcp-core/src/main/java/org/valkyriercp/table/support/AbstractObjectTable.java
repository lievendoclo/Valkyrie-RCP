 ...
 * mechanism because this class implements {@link org.springframework.context.ApplicationListener}.
...
public abstract class AbstractObjectTable extends AbstractControlFactory implements ApplicationListener {
	 ...
	 * @param finalEventList list to use
	 */
	 ...
	 * <p>
	 * <em>Note:</em> This method returns null unless {@link #getTable()} or is called
	...
	protected void init() {
		...
		objectSingularName = applicationConfig.messageResolver().getMessage(modelId + ".objectName.singular");
		objectPluralName = applicationConfig.messageResolver().getMessage(modelId + ".objectName.plural");
	}
	...
	protected void updateStatusBar() {
		if (statusBar != null) {
			...
			if (all == showing) {
				...
				msg = applicationConfig.messageResolver().getMessage(keys, new Object[] { "" + all, (all != 1) ? objectPluralName : objectSingularName });
			}
			else {
				String[] keys = new String[] { modelId + "." + SHOWINGN_MSG_KEY, SHOWINGN_MSG_KEY };

				msg = applicationConfig.messageResolver().getMessage(keys, new Object[] { "" + showing,
			...
			if (nselected > 0) {
				...
				msg += applicationConfig.messageResolver().getMessage(keys, new Object[] { "" + nselected });
			}
