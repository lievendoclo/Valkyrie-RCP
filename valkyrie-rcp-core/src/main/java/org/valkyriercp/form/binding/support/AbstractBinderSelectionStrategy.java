...
public abstract class AbstractBinderSelectionStrategy implements
	...
	public Binder selectBinder(FormModel formModel, String propertyName) {
		...
		Binder binder = findBinderByPropertyName(formModel.getFormObject()
				.getClass(), propertyName);
		if (binder == null) {
			// next try and find a binder for the specific property type
			binder = findBinderByPropertyType(getPropertyType(formModel,
					propertyName));
		}
		if (binder == null) {
		...
		throw new UnsupportedOperationException(
				"Unable to select a binder for form model [" + formModel
						+ "] property [" + propertyName + "]");
	}

	public Binder selectBinder(Class controlType, FormModel formModel,
			String propertyName) {
	 ...
	 * If no direct match found try to find binder for any superclass of the
	 * provided objectType which also has the same propertyName.
	 */
	protected Binder findBinderByPropertyName(Class parentObjectType,
			String propertyName) {
		PropertyNameKey key = new PropertyNameKey(parentObjectType,
				propertyName);
		Binder binder = (Binder) propertyNameBinders.get(key);
		if (binder == null) {
			// if no direct match was found try to find a match in any super
			// classes
			final Map potentialMatchingBinders = new HashMap();
			for (Iterator i = propertyNameBinders.entrySet().iterator(); i
					.hasNext();) {
				Map.Entry entry = (Map.Entry) i.next();
				if (((PropertyNameKey) entry.getKey()).getPropertyName()
						.equals(propertyName)) {
					potentialMatchingBinders.put(((PropertyNameKey) entry
							.getKey()).getParentObjectType(), entry.getValue());
				}
			}
			binder = (Binder) ClassUtils.getValueFromMapForClass(
					parentObjectType, potentialMatchingBinders);
			if (binder != null) {
				// remember the lookup so it doesn't have to be discovered again
				registerBinderForPropertyName(parentObjectType, propertyName,
						binder);
			}
		}
		return binder;
	...
	/**
	 * Try to find a binder for the provided propertyType. If no direct match
	 * found, try to find binder for closest superclass of the given control
	 * type.
	 */
	protected Binder findBinderByPropertyType(Class propertyType) {
		return (Binder) ClassUtils.getValueFromMapForClass(propertyType,
				propertyTypeBinders);
	}

	/**
	 * Try to find a binder for the provided controlType. If no direct match
	 * found, try to find binder for closest superclass of the given control
	 * type.
	 */
	protected Binder findBinderByControlType(Class controlType) {
		return (Binder) ClassUtils.getValueFromMapForClass(controlType,
				controlTypeBinders);
	}

	@Override
	public void registerBinderForPropertyName(Class parentObjectType,
			String propertyName, Binder binder) {
		propertyNameBinders.put(new PropertyNameKey(parentObjectType,
				propertyName), binder);
	}

	/**
	 * Add a list of binders that are bound to propertyNames. Each element in
	 * the list should be a Properties element describing the binder and
	 * propertyName. For more information about the structure of the properties
	 * see {@link #setBinderForPropertyName(Properties)}.
	 * 
	 * <br>
	 * <br>
	 * &lt;list&gt;<br>
	 ...
	 * 
	 * @param binders
	 *            List of <code>Properties</code> elements
	 ...
	 * </ul>
	 * <p>
	 * The first two keys are mandatory in combination with one of the two
	 * latter (binder or binderRef) The following two cases can be used to
	 * define a binder/propertyName combination: <br>
	 * <br>
	 * &lt;props&gt;<br>
	 ...
	 * 
	 * @param binder
	 *            The <code>Properties</code> object containing the correct
	 *            keys.
	 */
	 ...
	 * entry in the provided map using the key to specify the property type and
	 * the value to specify the binder.
	 * 
	 * <p>
	 * Binders specified in the provided map will override any binders
	 * previously registered for the same property type.
	 * 
	 * @param binders
	 *            the map containing the entries to register; keys must be of
	 *            type <code>Class</code> and values of type <code>Binder</code>
	 *            .
	 */
	 ...
	 * <p>
	 * Binders specified in the provided map will override any binders
	 * previously registered for the same control type.
	 * 
	 * @param binders
	 *            the map containing the entries to register; keys must be of
	 *            type <code>Class</code> and values of type <code>Binder</code>
	...
	public void setBindersForControlTypes(Map binders) {
		for (Iterator i = binders.entrySet().iterator(); i.hasNext();) {
			Map.Entry entry = (Map.Entry) i.next();
			registerBinderForControlType((Class) entry.getKey(),
					(Binder) entry.getValue());
		}
	}
	...
	protected boolean isEnumeration(FormModel formModel, String formPropertyPath) {
		// return LabeledEnum.class.isAssignableFrom(getPropertyType(formModel,
		// formPropertyPath));
		return getPropertyType(formModel, formPropertyPath).getClass().isEnum();
	}
	...
	private static class PropertyNameKey {
		...
		public PropertyNameKey(Class parentObjectType, String propertyName) {
			Assert.notNull(parentObjectType,
					"parentObjectType must not be null.");
		...
		public boolean equals(Object o) {
			...
			return propertyName.equals(propertyNameKey.propertyName)
					&& parentObjectType
							.equals(propertyNameKey.parentObjectType);
		}
